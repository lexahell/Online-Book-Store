...
const urlParams = new URLSearchParams(window.location.search);
    ...
    categoryBtns.forEach((btn) =>{
        btn.addEventListener("click",()=>{
            window.location.href = `../html/catalog.html?category=${btn.classList[1].toString()}`;
        });
        if(btn.classList[1].toString() ==  category.toString()){
            btn.classList.add("category-active"); 
        }
    });
    const indexJson = packageCategory.indexOf(category);
    const books = arrJsons[indexJson].data;
    for(let book of books){
        ...
        `;
        containerCatalog.insertAdjacentHTML("afterbegin",newProductString);
        const newProductCardSlide = containerCatalog.firstElementChild;
        ...
        newProductCardSlide.querySelector(".product-img").src = `..\\img\\books\\${category}\\${id}.jpg`;
