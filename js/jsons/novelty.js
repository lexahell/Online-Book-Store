const novelty = {
  data: [
    {
      attributes: {
        authors: [
          {
            firstName: "Тунсю",
            id: 569771,
            lastName: "Мосян",
            middleName: "",
            url: "mosyan-tunsyu",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "2996776",
        description:
          "ОДИН ЧЕЛОВЕК. ВСЕГО ОДИН. НА САМОМ ДЕЛЕ ЭТОГО ДОСТАТОЧНО.\nПутешествие к Медной Печи становится всё более непредсказуемым. По дороге к вулкану Се Лянь узнаёт шокирующую правду о Хуа Чэне, но герои даже не успевают объясниться, ведь Поднебесной грозит страшная опасность — Безликий Бай вернулся! Се Ляню предстоит разгадать тайну личности демона и отразить новую волну поветрия ликов, но перед этим он столкнётся с призраками прошлого…\nЧто произошло с наследным принцем после первого низвержения? Как он всё потерял и опустился на самое дно? Какие страдания выпали на его долю? Кто заставил его блуждать во тьме, а кто помог обратиться к свету?\n\n*Бонус для всего тиража:\nВкладыш с игрой «Одень Се Ляня» от Паб Линца.",
        discount: "9",
        exclusive: false,
        id: 2996776,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1375,
        originalPicture: "/nomenclature/29/967/2996776-2.jpg",
        pages: "",
        picture: "/nomenclature/29/967/2996776-2.jpg",
        price: 1250,
        publisher: {
          id: 132549,
          slug: "komilfo",
          title: "КОМИЛЬФО",
        },
        rating: {
          count: "4.48",
          votes: 2942,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Благословение небожителей. Том 5",
        url: "product/blagoslovenie-nebozhiteley-tom-5-2996776",
        yearPublishing: 2023,
      },
      id: "2996776",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Гэгэ",
            id: 573581,
            lastName: "Акутами",
            middleName: "",
            url: "akutami-gege",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3007990",
        description:
          "Масштабная битва в районе Сибуя осталась позади... Маги потерпели разгромное поражение, и на волю вырвалась армия злобных духов, погрузившая Токио в хаос. Кроме того, проклинатель по имени Кэндзяку пробудил в тысяче людей магические способности, чтобы привести свой коварный план в жизнь. \nЮдзи Итадори и его товарищи спешат остановить негодяя, но получится ли у них? Ведь теперь, когда Сатору Годзё, могущественный из магов, запечатан во Вратах Заключения, заклинателям остается полагаться только на собственные силы...",
        discount: "9",
        exclusive: false,
        id: 3007990,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 926,
        originalPicture: "/nomenclature/30/079/3007990-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007990-2.jpg",
        preOrderDate: "2023-11-09T00:00:00+03:00",
        price: 842,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.69",
          votes: 150,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Магическая битва. Книга 9. Перелетный гусь. Пламя",
        url: "product/magicheskaya-bitva-kniga-9-pereletnyy-gus-plamya-3007990",
        yearPublishing: 2023,
      },
      id: "3007990",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Ася",
            id: 566987,
            lastName: "Лавринович",
            middleName: "",
            url: "lavrinovich-asya",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110016,
          slug: "lyubovnye-romany",
          title: "Любовные романы",
          url: "lyubovnye-romany",
        },
        coauthors: [],
        code: "3008745",
        description:
          "Новогодняя новинка от Аси Лавринович!\n\nВсе девчонки в школе без ума от Вадима Рубцова — высокого зеленоглазого блондина с обаятельной улыбкой. Но кто бы мог подумать, что обычная старшеклассница Томила Литвинова станет его девушкой. Правда, они сходили всего на два свидания и даже ни разу не целовались.\nЗа несколько дней до Нового года Тома получила неожиданное сообщение: Вадима похитили и просят выкуп. Вот так поворот!\nУ Томы будет всего неделя на то, чтобы спасти своего парня из лап бандитов. Но как собрать огромную сумму? Помочь ей сможет только Егор Корниенко, которого она просто терпеть не может.\n\n«Эта история заставит ваши сердца трепетать от любви и умиления. Она также привнесет капельку грусти, ведь не всегда желаемое является действительным». — Маша, книжный блогер. ТГ Хижина Ежа | ms.hedgehog\n\nОб авторе\nАся Лавринович — один из самых популярных авторов российского янг эдалта в жанре современной сентиментальной прозы. Суммарный тираж ее проданных книг составляет более 650 000 экземпляров. На две ее книги — «Загадай любовь» и «Нелюбовь сероглазого короля» — куплены права на экранизацию.\n\n* На территории России только в магазинах «Читай-город», в интернет-магазине www.chitai-gorod.ru, в магазинах «Буквоед», в интернет-магазине https://www.bookvoed.ru/, в интернет-магазине https://book24.ru/ в период с 12.10.2023 по 27.11.2023 г. включительно.",
        discount: "9",
        exclusive: true,
        id: 3008745,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 623,
        originalPicture: "/nomenclature/30/087/3008745-2.jpg",
        pages: "",
        picture: "/nomenclature/30/087/3008745-2.jpg",
        preOrderDate: "2023-11-09T00:00:00+03:00",
        price: 566,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.31",
          votes: 112,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Сказка о снежной принцессе",
        url: "product/skazka-o-snezhnoy-princesse-3008745",
        yearPublishing: 2024,
      },
      id: "3008745",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Эл",
            id: 20114043,
            lastName: "Моргот",
            middleName: "",
            url: "morgot-el",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "2998182",
        description:
          'Шена, продолжающего притворяться грозным старейшиной пика Черного лотоса, обвиняют в череде загадочных убийств в ордене. Его репутация уже настолько подпорчена, что никому и в голову не приходит провести расследование. Но что, если в этот раз за убийствами девушек действительно стоял не он? Система тем временем начинает вести себя крайне подозрительно, а Шен погружается в водоворот семейных тайн и интриг, которые подводят его на шаг ближе к пониманию своей новой личности. Для кого эта книга Для поклонников азиатского фэнтези и китайских новелл. Для тех, кто ищет атмосферу мистики и сказки, созданной из переплетения китайской и европейской мифологий. Для любителей фэнтези-дорам в духе "Неукротимый: Повелитель Чэньцин", аниме "Магистр дьявольского культа" и поклонников новелл Мосян Тунсю и других авторов сянься. Для тех, кому нравятся сильные эмоции и неожиданные повороты сюжета. Для тех, кого покорили прекрасные заклинатели, летающие на мечах и совершенствующие тело и дух, чтобы жить вечно.',
        discount: "19",
        exclusive: false,
        id: 2998182,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1299,
        originalPicture: "/nomenclature/29/981/2998182-4.jpg",
        pages: "",
        picture: "/nomenclature/29/981/2998182-4.jpg",
        preOrderDate: "2023-11-04T00:00:00+03:00",
        price: 1058,
        publisher: {
          id: 135192,
          slug: "mann-ivanov-i-ferber",
          title: "Манн, Иванов и Фербер",
        },
        rating: {
          count: "4.55",
          votes: 272,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Злодейский путь!.. Том 3",
        url: "product/zlodeyskiy-put-tom-3-2998182",
        yearPublishing: 2023,
      },
      id: "2998182",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "",
            id: 6816182,
            lastName: "Мильчха",
            middleName: "",
            url: "milchha",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3006764",
        description:
          "Лериана Макмиллан постепенно набирает вес в светском обществе. О её сделке с герцогом Ноа Виннайтом становится известно королю, который хочет лично убедиться, что она достойная невеста для его брата.\n\nДевушка начинает сомневаться в правильности своих поступков, но ещё не до конца осознаёт, насколько сильно она вмешалась в судьбы своих новых друзей и врагов...\n\nНа кону теперь не только её сделка с герцогом, но и снова её собственная жизнь в роли второстепенной героини.",
        discount: "25",
        exclusive: false,
        id: 3006764,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1271,
        originalPicture: "/nomenclature/30/067/3006764-5.jpg",
        pages: "",
        picture: "/nomenclature/30/067/3006764-5.jpg",
        preOrderDate: "2023-11-01T00:00:00+03:00",
        price: 953,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.56",
          votes: 66,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Лериана, невеста герцога по контракту. Том 4 (со стикерпаком)",
        url: "product/leriana-nevesta-gercoga-po-kontraktu-tom-4-so-stikerpakom-3006764",
        yearPublishing: 2023,
      },
      id: "3006764",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Масаси",
            id: 569129,
            lastName: "Кисимото",
            middleName: "",
            url: "kisimoto-masasi",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3007930",
        description:
          "Встреча на мосту Тэнти сразу пошла не по плану… Таинственный шпион оказался не кем иным, как Кабуто. Он уже давно не подчинялся «Рассвету» и действовал исключительно в интересах своего господина – Оротимару, который и сам вскоре появился на мосту. Увидев своего давнего врага, Наруто потерял голову. Он даже не пытался контролировать силу Девятихвостого и в итоге принял форму лисадемона. В грандиозном и жутком сражении не нашлось победителя. Наруто пришел в себя лишь \nблагодаря технике командира Ямато. А Сай тем временем предложил Оротимару сделку… Возможно ли, что новенький, рьяно защищавший Листву, решил предать деревню?.. \n\nВ двенадцатую книгу вошли главы 300–329.",
        discount: "9",
        exclusive: false,
        id: 3007930,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1090,
        originalPicture: "/nomenclature/30/079/3007930-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007930-2.jpg",
        price: 991,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.58",
          votes: 55,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Naruto. Наруто. Книга 12. Встреча после разлуки!",
        url: "product/naruto-naruto-kniga-12-vstrecha-posle-razluki-3007930",
        yearPublishing: 2023,
      },
      id: "3007930",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Брендон",
            id: 569654,
            lastName: "Сандерсон",
            middleName: "",
            url: "sanderson-brendon",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3005405",
        description:
          "На планете, где в океанах не вода, а чрезвычайно опасные организмы, на острове посреди Изумрудного моря живет юноша, которому совсем не в радость его судьба — судьба единственного сына герцога, будущего наследника престола. И однажды Чарли приходится покинуть остров: отец увозит его на поиски знатной невесты, чтобы заключить выгодный династический брак. Прощаясь, Чарли обещает юной простолюдинке, в которую влюблен, вернуться и жениться именно на ней. Обещает — и не возвращается. Вместо него герцог привозит совсем другого юношу, своего дальнего родственника, которого и объявляет своим приемным сыном и наследником. Однако девушке все же удается выяснить, что Чарли был брошен в Полуночном море, самом опасном из двенадцати морей планеты. И теперь, чтобы отправиться к нему на выручку, мало нарушить строжайший запрет герцога покидать остров — необходимо изобрести способ его нарушить… С цветными иллюстрациями знаменитого фэнтезийного художника Говарда Лайона! Впервые на русском!",
        discount: "27",
        exclusive: false,
        id: 3005405,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1499,
        originalPicture: "/nomenclature/30/054/3005405-2.jpg",
        pages: "",
        picture: "/nomenclature/30/054/3005405-2.jpg",
        price: 1094,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.60",
          votes: 70,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Локон с Изумрудного моря",
        url: "product/lokon-s-izumrudnogo-morya-3005405",
        yearPublishing: 2023,
      },
      id: "3005405",
      type: "product",
    },
    {
      attributes: {
        authors: [],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3000140",
        description:
          "Мир изменился. Повсюду открылись Врата, соединяющие реальность с параллельным измерением. Некоторые люди получили сверхспособности и стали охотниками на монстров, которые скрываются в подземельях и угрожают человечеству.  Сон Чину — слабейший и невезучий охотник Е-ранга, вынужденный рисковать своей жизнью, чтобы заработать денег для оплаты больничных счетов матери и обучения сестры. Одним роковым днём он оправляется в очередной рейд в низко-уровневое подземелье с группой охотников, но никто из них даже представить не мог, с каким ужасом они столкнутся внутри…",
        discount: "25",
        exclusive: false,
        id: 3000140,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1463,
        originalPicture: "/nomenclature/30/001/3000140-11.jpg",
        pages: "",
        picture: "/nomenclature/30/001/3000140-11.jpg",
        preOrderDate: "2023-11-03T00:00:00+03:00",
        price: 1097,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.46",
          votes: 188,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Поднятие уровня в одиночку. Solo Leveling. Том 1",
        url: "product/podnyatie-urovnya-v-odinochku-solo-leveling-tom-1-3000140",
        yearPublishing: 2023,
      },
      id: "3000140",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Джин",
            id: 21617651,
            lastName: "Соул",
            middleName: "",
            url: "soul-dzhin",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3004902",
        description:
          "Небесный дворец встретил Ху Фэйциня настоящим хаосом и ужасной опасностью. Тот, кого некогда звали Господином-с-горы, должен найти способ предотвратить войну двух миров. Но кто поможет ему? Ху Вэй? Загадочная Тьма, о природе которой Ху Фэйциню ещё предстоит узнать? Или странный голос из темноты, назвавшийся Лао Луном? Кто из них друг, а кто враг – решать только ему самому.",
        discount: "25",
        exclusive: false,
        id: 3004902,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 594,
        originalPicture: "/nomenclature/30/049/3004902-5.jpg",
        pages: "",
        picture: "/nomenclature/30/049/3004902-5.jpg",
        preOrderDate: "2023-11-22T00:00:00+03:00",
        price: 445,
        publisher: {
          id: 125802,
          slug: "zhanry",
          title: "Жанры",
        },
        rating: {
          count: "4.85",
          votes: 46,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Девять хвостов бессмертного мастера. Том 2",
        url: "product/devyat-hvostov-bessmertnogo-mastera-tom-2-3004902",
        yearPublishing: 2023,
      },
      id: "3004902",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Донато",
            id: 569144,
            lastName: "Карризи",
            middleName: "",
            url: "karrizi-donato",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110011,
          slug: "detektivnyj-roman-triller",
          title: "Детективный роман. Триллер",
          url: "detektivnyj-roman-triller",
        },
        coauthors: [],
        code: "3007935",
        description:
          "Донато Карризи — известный итальянский писатель и сценарист, специалист в области криминалистики и поведенческих наук, лауреат итальянской премии Bancarella (2009), французской премии Prix SNCF du polar (2011) и других, автор бестселлеров, переведенных на 30 с лишним языков и расходящихся многомиллионными тиражами. Три свои книги — «Девушка в тумане» (2017), «Девушка в лабиринте» (2019) и «Я — бездна» (2022) — Карризи сам экранизировал. «Дом огней» — продолжение его блестящих романов «Дом голосов» и «Дом без воспоминаний». В доме на холме живет десятилетняя девочка по имени Эва — без родителей, но с домоправительницей и финской студенткой, она же няня, Майей Сало. А еще у Эвы есть воображаемый друг, и то, что говорит и делает этот друг, вызывает тревогу. Майя обращается к Пьетро Джерберу, флорентийскому гипнотисту, который работает с травмированными детьми, — тот, правда, и сам после событий предыдущих романов не в лучшем душевном состоянии, однако помочь Эве больше некому. Джербер берется за дело — и выясняется, что Эвин воображаемый друг знает о Джербере слишком много. Этот таинственный друг знает даже обстоятельства, которые сопутствовали смерти Джербера много-много лет назад… Впервые на русском!",
        discount: "9",
        exclusive: false,
        id: 3007935,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 754,
        originalPicture: "/nomenclature/30/079/3007935-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007935-2.jpg",
        price: 685,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.57",
          votes: 30,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Дом огней",
        url: "product/dom-ogney-3007935",
        yearPublishing: 2023,
      },
      id: "3007935",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Тесса",
            id: 15033576,
            lastName: "Бейли",
            middleName: "",
            url: "beyli-tessa",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110016,
          slug: "lyubovnye-romany",
          title: "Любовные романы",
          url: "lyubovnye-romany",
        },
        coauthors: [],
        code: "3007076",
        description:
          "Тейлор и ее брат арендуют небольшой коттедж в солнечном Кейп-Коде, чтобы провести там летние каникулы. Их отдых прерывается, когда по соседству происходит громкое убийство. Расследует преступление Майлз – суровый парень, который отказывается принимать помощь Тейлор в расследовании. Она работает учительницей в начальной школе, а в свободное время увлекается тру-крайм подкастами, поэтому ей очень хочется помочь найти преступника. Позволит ли Майлз присоединиться Тейлор к расследованию? Ведь с каждым днем его влечет к этой девушке все сильнее.",
        discount: "9",
        exclusive: false,
        id: 3007076,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 570,
        originalPicture: "/nomenclature/30/070/3007076-2.jpg",
        pages: "",
        picture: "/nomenclature/30/070/3007076-2.jpg",
        preOrderDate: "2023-10-31T00:00:00+03:00",
        price: 518,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "3.96",
          votes: 48,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Мои убийственные каникулы",
        url: "product/moi-ubiystvennye-kanikuly-3007076",
        yearPublishing: 2023,
      },
      id: "3007076",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Кора",
            id: 8290219,
            lastName: "Рейли",
            middleName: "",
            url: "reyli-kora",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110002,
          slug: "sovremennaya-proza",
          title: "Современная проза",
          url: "sovremennaya-proza",
        },
        coauthors: [],
        code: "3006769",
        description:
          "Маттео – младший брат Луки, дона нью-йоркской мафии, и его правая рука. Пижон, провокатор и ловелас, он может поставить на колени любого.\nМаттео страшно заинтригован дерзкой, склонной к авантюрам красавицей Джианной Скудери. Она обещана ему в жены, только Джианна сбегает от него в Европу. Но ведь ее разыскивает один из их лучших охотников и убийц – Маттео Витиелло.\nВ итоге Джианна вынуждена выйти замуж за Маттео, при этом она всеми силами пытается превратить его жизнь в ад, а Маттео мастер интеллектуальных игр и во что бы то ни стало стремится приручить рыжую бестию.\nЧем сильнее Джианна его ненавидит, тем больше он жаждет ее подчинить себе.\nЧем закончится их поединок? И кто же из них по-настоящему попал в западню?",
        discount: "9",
        exclusive: false,
        id: 3006769,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 649,
        originalPicture: "/nomenclature/30/067/3006769-2.jpg",
        pages: "",
        picture: "/nomenclature/30/067/3006769-2.jpg",
        preOrderDate: "2023-11-04T00:00:00+03:00",
        price: 590,
        publisher: {
          id: 125802,
          slug: "zhanry",
          title: "Жанры",
        },
        rating: {
          count: "4.18",
          votes: 91,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Связанные ненавистью",
        url: "product/svyazannye-nenavistyu-3006769",
        yearPublishing: 2023,
      },
      id: "3006769",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Харуко",
            id: 23140859,
            lastName: "Итикава",
            middleName: "",
            url: "itikava-haruko",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3007991",
        description:
          "Самоцветам с низкой твердостью ничего не остается, кроме как проявлять мужество. \nДалекое будущее. Бессмертные драгоценности продолжают воевать с лунянами. Пока большая часть самоцветов пребывает в зимней спячке, главная героиня Фосфофиллит объединяется с Антарктицит — минералом, активным лишь зимой. \nПеред вами история о сильных, хрупких и прекрасных драгоценностях, которые сражаются за свою жизнь. \n\nВо вторую книгу вошли главы с 14-й по 28-ю, а также две бонусные истории.",
        discount: "9",
        exclusive: false,
        id: 3007991,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1075,
        originalPicture: "/nomenclature/30/079/3007991-3.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007991-3.jpg",
        price: 977,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.83",
          votes: 70,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Страна самоцветов. Книга 2",
        url: "product/strana-samocvetov-kniga-2-3007991",
        yearPublishing: 2023,
      },
      id: "3007991",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Сора",
            id: 501963,
            lastName: "Наумова",
            middleName: "",
            url: "naumova-sora",
          },
          {
            firstName: "Мария",
            id: 595864,
            lastName: "Дубинина",
            middleName: "",
            url: "dubinina-mariya",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3012811",
        description:
          "Зачаровывающая магия древней Японии, невероятная притягательность колдунов, демонов и обычных людей, снисхождение и гнев богов — все это в новом увлекательном цикле, основанном на легендах, берущих свое начало у истоков времен. \n\nВ полном чудес и опасностей мире для юных экзорцистов всегда найдется дело. Ученики трёх великих школ оммёдо продолжают погоню за своим одержимым другом, а меж тем над империей Ямато сгущаются тучи. Мацумото Хизаши ясно видит — их с Кентой жизни находятся в самом центре темного водоворота. Разгораются костры Ёми, к людям готовится прийти ложный бог, и прошлое накладывается на настоящее, тревожа память и сердце.\n\nХизаши ведут вперёд стыд и вина, однако конец у выбранного им пути может оказаться спуском в бездну.\n\nВторая книга серии. Путешествие по удивительному, коварному и чудесному миру, полному тайн, интриг и настоящей мужской дружбы продолжается!",
        discount: "26",
        exclusive: false,
        id: 3012811,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 599,
        originalPicture: "/nomenclature/30/128/3012811-1.jpg",
        pages: "",
        picture: "/nomenclature/30/128/3012811-1.jpg",
        price: 443,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.89",
          votes: 9,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Серебряный змей в корнях сосны - 2",
        url: "product/serebryanyy-zmey-v-kornyah-sosny-2-3012811",
        yearPublishing: 2024,
      },
      id: "3012811",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Лия",
            id: 569162,
            lastName: "Арден",
            middleName: "",
            url: "arden-liya",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3007077",
        description:
          "Коллекционное издание 2024 года! Мама нарекла меня Ярой, в честь яркого солнца, которое отражается от снежного покрова. Но откуда ей было знать, как выглядит снег, если зимы нет уже сотни лет? Единственное место, где ещё сохранились холода — это проклятый лес. Там живёт последний декабрьский колдун, из-за которого зима может однажды вернуться. Люди всеми силами пытаются этому помешать, принося в жертву детей, рождённых в декабре. И я оказалась одной из этих детей.",
        discount: "15",
        exclusive: false,
        id: 3007077,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1299,
        originalPicture: "/nomenclature/30/070/3007077-1.jpg",
        pages: "",
        picture: "/nomenclature/30/070/3007077-1.jpg",
        preOrderDate: "2023-11-02T00:00:00+03:00",
        price: 1104,
        publisher: {
          id: 1519257,
          slug: "chernym-belo",
          title: "Черным-бело",
        },
        rating: {
          count: "4.34",
          votes: 119,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Невеста Ноября",
        url: "product/nevesta-noyabrya-3007077",
        yearPublishing: 2024,
      },
      id: "3007077",
      type: "product",
    },

    {
      attributes: {
        authors: [
          {
            firstName: "Амели",
            id: 5157539,
            lastName: "Вэнь Чжао",
            middleName: "",
            url: "ven-chzhao-ameli",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3009583",
        description:
          "Новая дилогия Амели Вэнь Чжао, автора серии «Магия крови»!\nВ павшем королевстве у одной девушки есть ключ к разгадке тайн прошлого своего народа и спасению от демонов.\nФэнтези, вдохновленное мифологией и фольклором Древнего Китая.\nКогда-то у Лань было другое имя, но затем элантийцы вторглись в ее королевство, убили мать девушки, а магию объявили вне закона. Теперь по ночам Лань поет на улицах Хаак Гуна, а днем разгадывает загадочное появление метки на руке. Таинственный знак — иероглиф, который никто кроме Лань не может увидеть.\nОднажды ночью, в попытках скрыться от страшной судьбы, девушка встречает таинственного юношу. Зен — один из легендарных магов Последнего царства. Ходили слухи, что эту магию даровали демоны, однако теперь Зену приходится спасаться, чтобы скрыть свой дар от элантийцев.\nКогда Зен находит Лань, он узнает, что девушка обладает способностью, запечатанной в ее метке и скрытой от чужих глаз. Вместе они прокладывают путь к Туманным горам Последнего царства, к ордену мастеров, планирующим свергнуть режим Элантии. Да начнется битва за Последнее Королевство!\n№ 29 в категории бестселлеров мультикультурной художественной литературы для молодежи на Amazon.com\n«Захватывающее эпическое фэнтези… не могу дождаться продолжения». — Тань Сью Линн, автор дилогии «Дочь Лунной богини»",
        discount: "9",
        exclusive: false,
        id: 3009583,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 812,
        originalPicture: "/nomenclature/30/095/3009583-4.jpg",
        pages: "",
        picture: "/nomenclature/30/095/3009583-4.jpg",
        preOrderDate: "2023-11-10T00:00:00+03:00",
        price: 738,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.26",
          votes: 34,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Песнь серебра, пламя, подобное ночи",
        url: "product/pesn-serebra-plamya-podobnoe-nochi-3009583",
        yearPublishing: 2024,
      },
      id: "3009583",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Али",
            id: 6468244,
            lastName: "Хейзелвуд",
            middleName: "",
            url: "heyzelvud-ali",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110002,
          slug: "sovremennaya-proza",
          title: "Современная проза",
          url: "sovremennaya-proza",
        },
        coauthors: [],
        code: "2999738",
        description:
          "Мэллори Гринлиф бросила шахматы, когда они разрушили ее семью. Но четыре года спустя она все же соглашается принять участие в благотворительном турнире и случайно обыгрывает Нолана Сойера — действующего чемпиона и настоящего «бэд боя» в мире шахмат.\nПроигрыш Нолана новичку шокирует всех, а громкая победа открывает Мэллори двери к денежному призу, который так ей необходим. Мэллори сомневается, стоит ли идти на сделку с совестью, но Сойер серьезно намерен отыграться и не желает выпускать загадочную соперницу из поля зрения.\n\nВступая в большую игру, Мэллори изо всех сил старается оградить свою семью от того, что когда-то ее разрушило, и заново не влюбиться в шахматы. Однако вскоре понимает, что не может перестать думать о Сойере, ведь он не только умен, но и чертовски привлекателен…",
        discount: "21",
        exclusive: false,
        id: 2999738,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 799,
        originalPicture: "/nomenclature/29/997/2999738-3.jpg",
        pages: "",
        picture: "/nomenclature/29/997/2999738-3.jpg",
        preOrderDate: "2023-11-15T00:00:00+03:00",
        price: 633,
        publisher: {
          id: 135192,
          slug: "mann-ivanov-i-ferber",
          title: "Манн, Иванов и Фербер",
        },
        rating: {
          count: "4.33",
          votes: 132,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Шах и мат",
        url: "product/shah-i-mat-2999738",
        yearPublishing: 2023,
      },
      id: "2999738",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "",
            id: 9039855,
            lastName: "Суволь",
            middleName: "",
            url: "suvol",
          },
          {
            firstName: "Квон",
            id: 9039858,
            lastName: "Гёыль",
            middleName: "",
            url: "geyl-kvon",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "2998184",
        description:
          "Четвертая часть популярной манхвы, где главная героиня перенеслась в онлайн-игру и стала Пенелопой Экхарт – главной злодейкой, которой все желают смерти.\n\nПенелопа хочет сделать Иклису подарок, и во время ежегодных охотничьих состязаний отправляется в лес на поиски добычи. Однако каждая попытка поймать зверька заканчивается неудачей. И тут внезапно девушка сталкивается… с огромным медведем!\nВ последний момент злодейке на помощь приходит кронпринц Каллисто. Но вскоре жизни героев снова оказываются под угрозой — на них нападает таинственный враг. Смогут ли герои спастись на этот раз?",
        discount: "9",
        exclusive: false,
        id: 2998184,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1329,
        originalPicture: "/nomenclature/29/981/2998184-7.jpg",
        pages: "",
        picture: "/nomenclature/29/981/2998184-7.jpg",
        preOrderDate: "2023-11-14T00:00:00+03:00",
        price: 1208,
        publisher: {
          id: 135192,
          slug: "mann-ivanov-i-ferber",
          title: "Манн, Иванов и Фербер",
        },
        rating: {
          count: "4.50",
          votes: 122,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Единственный конец злодейки - смерть. Том 4",
        url: "product/edinstvennyy-konec-zlodeyki-smert-tom-4-2998184",
        yearPublishing: 2023,
      },
      id: "2998184",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Софи",
            id: 426124,
            lastName: "Анри",
            middleName: "",
            url: "anri-sofi",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3005599",
        description:
          "Продолжение невероятного фэнтези-романа от российской писательницы Софи Анри — история королевства Арден и его Хранителя, Рэндалла, а также княжны Авроры, гордой северянки. Дворцовые интриги, любовь и ненависть, войны, скелеты в шкафу, семейные тайны и жуткие ритуалы. \n \n Прошел год с тех пор, как Рэндалл и Аврора скрепили свой союз древним ритуалом, а их души стали единым целым. Но близится война, и Рэндалл, прощаясь с женой, уезжает в Восточное Королевство. Корабль терпит крушение. Убитая горем Аврора оплакивает мужа, но в Ардене ей нужно быть сильной, чтобы защитить их сына — последнего потомка рода Корвин — от паутины дворцовых интриг, и единственное, что помогает ей не сломаться — это перстень, чей свет таит в себе надежду. \n \n В новом томе вы узнаете больше о телохранителе Рэндалла, Закарии, и об Ордене теней, хранящем темные тайны.  Вас ждет магический мир, в котором правят богатство и сила чистой крови, в воздухе витают следы древней магии, а любовь может или спасти героев, или погубить.\n \n Всего за несколько недель дебютный роман писательницы «Принц Ардена» взлетел на первые строчки рейтингов и занял 5-е место в рейтинге жанра «Романтическое отечественное фэнтези» в июне 2023 в «Лабиринте»! До момента издания книга набрала более 7 тыс. просмотров на платформе Wattpad и завоевала сердца многих читателей.\n \n Южное королевство можно сравнить с Францией эпохи Людовика XIV, а Северное царство — со Скандинавией или средневековой Шотландией. Атмосфера альтернативной эпохи Возрождения приведет вас в восторг!",
        discount: "9",
        exclusive: false,
        id: 3005599,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 782,
        originalPicture: "/nomenclature/30/055/3005599-4.jpg",
        pages: "",
        picture: "/nomenclature/30/055/3005599-4.jpg",
        preOrderDate: "2023-10-31T00:00:00+03:00",
        price: 711,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.58",
          votes: 84,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Хранитель Ардена",
        url: "product/hranitel-ardena-3005599",
        yearPublishing: 2024,
      },
      id: "3005599",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Урса",
            id: 23166177,
            lastName: "Рысина",
            middleName: "",
            url: "rysina-ursa",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110016,
          slug: "lyubovnye-romany",
          title: "Любовные романы",
          url: "lyubovnye-romany",
        },
        coauthors: [],
        code: "3006762",
        description:
          "Юная Никкаль отправляется на поиски пропавшего брата. Ей предстоит вернуть утраченную магию, сразиться с лесными чудовищами и собственными демонами, выйти на след культа и узнать правду о древних богах.\nВместе с Ниаллом — ректором магической академии, — и Сином — могущественным незнакомцем из лесной чащи — Никкаль раскроет секреты прошлого своей семьи и постарается предотвратить надвигающуюся катастрофу.\n\n*Подарок при покупке каждой книги: эксклюзивная открытка. Количество подарков ограничено.",
        discount: "25",
        exclusive: true,
        id: 3006762,
        inSubscription: false,
        isBook: true,
        isBookmarks: true,
        new: true,
        oldPrice: 798,
        originalPicture: "/nomenclature/30/067/3006762-4.jpg",
        pages: "",
        picture: "/nomenclature/30/067/3006762-4.jpg",
        preOrderDate: "2023-10-30T00:00:00+03:00",
        price: 598,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.01",
          votes: 471,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Клуб Романтики. Цветок из огня Тиамат",
        url: "product/klub-romantiki-cvetok-iz-ognya-tiamat-3006762",
        yearPublishing: 2023,
      },
      id: "3006762",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Ака",
            id: 566615,
            lastName: "Акасака",
            middleName: "",
            url: "akasaka-aka",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3007941",
        description:
          "Очередной учебный год в Японии подошел к концу, и для старшеклассников настала пора выпускных. Перед тем как навсегда покинуть школу, Цубамэ намерена ответить на признание Исигами. Любовная история Ю и Коясу близится к завершению!\nМежду тем в академию Сютиин перевелся новый ученик – Микадо Сидзё. Сын главы крупной корпорации, многообещающий спортсмен и блестящий ученик – казалось бы, он идеален во всем. Вот только даже у таких любимчиков жизни есть свои страхи. Например, стать изгоем в новой школе… \nТем временем Сироганэ и Кагуя продолжают постигать науку романтических отношений. И на этот раз сладкой парочке предстоит перелистнуть их «Азбуку любви» на новую страницу. Интересно, насколько далеко они осмелятся зайти?",
        discount: "9",
        exclusive: false,
        id: 3007941,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1056,
        originalPicture: "/nomenclature/30/079/3007941-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007941-2.jpg",
        price: 960,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.39",
          votes: 23,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title:
          "Госпожа Кагуя: В любви как на войне: Любовная битва двух гениев. Книга 11",
        url: "product/gospozha-kaguya-v-lyubvi-kak-na-voyne-lyubovnaya-bitva-dvuh-geniev-kniga-11-3007941",
        yearPublishing: 2023,
      },
      id: "3007941",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Евгений",
            id: 23218361,
            lastName: "Павлов",
            middleName: "",
            url: "pavlov-evgeniy",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110300,
          slug: "yazyki-i-sredy-programmirovaniya",
          title: "Языки и среды программирования",
          url: "yazyki-i-sredy-programmirovaniya",
        },
        coauthors: [],
        code: "3006238",
        description:
          "Евгений Павлов – IT-специалист с 25-летним стажем, преподаватель обучающих курсов по аналитике и разработке игр, гость «Разведопроса» и спикер круглого стола Non/fiction.\nОчень хочется стать программистом и войти в IT, но не знаете с чего начать? Поздравляю, в ваших руках учебник нового поколения – без кучи непонятных слов и кодов расскажем про инструменты и библиотеки для программирования на Python. И все это заправлено лайфхаками, вдохновляющими историями из мира настоящих хакеров и игрофикацией – потому что обучение не должно быть скучным.\n\n«Знакомьтесь, Python» – уверенный первый шаг в мир программирования. Получите базовые знания о сфере IT, а также найдете ответы на вопросы по типу: с чего начать, как выбрать профессию, где прокачать скиллы и как подготовиться к собеседованию так, чтобы работодатель предложил оффер.",
        discount: "26",
        exclusive: false,
        id: 3006238,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 849,
        originalPicture: "/nomenclature/30/062/3006238-2.jpg",
        pages: "",
        picture: "/nomenclature/30/062/3006238-2.jpg",
        preOrderDate: "2023-11-16T00:00:00+03:00",
        price: 632,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.31",
          votes: 13,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Знакомьтесь, Python. Секреты профессии",
        url: "product/znakomtes-python-sekrety-professii-3006238",
        yearPublishing: 2023,
      },
      id: "3006238",
      type: "product",
    },
    {
      attributes: {
        authors: [],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3009030",
        description:
          "«Некромант-одиночка» официально в России.\nПопулярная новелла «Некромант-одиночка», по которой существует одноименная манхва. В России ее прочитало 3,4 млн человек.\n\nМонстры из легенд теперь населяют город, который напоминает онлайн-игру.\nЭто был самый обычный учебный день, пока в воздухе не начали вращаться загадочные карты с изображениями воинов, магов и лучников. Студентов охватила паника, но только не Ю Сону, который бессознательно выбрал карту и встал на путь некроманта. Теперь он обладатель редкой способности, которая дает ему возможность управлять нежитью. Пока юноша наращивает силу, его враги не дремлют. Поскольку кампус и внешний мир на пороге хаоса, Ю Сону нужно использовать свои способности, чтобы подчинить себе монстров и спасти друзей. Но всем ли удастся выжить?\n\nНовелла и манхва получили громкое признание:\n- Более 2 млн прочтений на корейской платформе Kakao.\n- Один из популярных проектов корейского портала Naver.\n- Один из хитов корейской платформы Webtoons.\n- На российском сайте RanobeLib рейтинг 9.25 из 10.\n\n*На территории России только в магазинах «Читай-город», в интернет-магазине www.chitai-gorod.ru, в магазинах «Буквоед», в интернет-магазине https://www.bookvoed.ru/, в интернет-магазине https://book24.ru/ в период с 13.10.2023 по 26.11.2023 г. вкл.",
        discount: "18",
        exclusive: true,
        id: 3009030,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 729,
        originalPicture: "/nomenclature/30/090/3009030-5.jpg",
        pages: "",
        picture: "/nomenclature/30/090/3009030-5.jpg",
        preOrderDate: "2023-11-10T00:00:00+03:00",
        price: 600,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.72",
          votes: 32,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Некромант-одиночка: Новелла. Том 1",
        url: "product/nekromant-odinochka-novella-tom-1-3009030",
        yearPublishing: 2024,
      },
      id: "3009030",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Хью",
            id: 463585,
            lastName: "Хауи",
            middleName: "",
            url: "haui-hyu",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110008,
          slug: "nauchnaya-fantastika",
          title: "Научная фантастика",
          url: "nauchnaya-fantastika",
        },
        coauthors: [],
        code: "3007976",
        description:
          "С чего все начиналось. \nГод 2049-й, Вашингтон, округ Колумбия. Пол Турман, сенатор, приглашает молодого конгрессмена Дональда Кини, архитектора по образованию, для участия в специальном проекте под условным названием КЛУ (Комплекс по локализации и утилизации). Суть проекта — создание подземного хранилища для ядерных и токсичных отходов, а Дональду поручается спроектировать бункер-укрытие для обслуживающего персонала объекта. \nГод 2052-й , округ Фултон, штат Джорджия. Проект завершен. И словно бы как кульминация к его завершению, Америку накрывает серия ядерных ударов. Турман, Дональд и другие избранные представители американского общества перемещаются в обустроенное укрытие. Тут-то Кини и открывается суровая и страшная истина: КЛУ был всего лишь завесой для всемирной операции «Пятьдесят», цель которой — сохранить часть человечества в случае ядерной катастрофы. А цифра 50 означает количество возведенных укрытий, управляемых из командного центра укрытия № 1. \nЧем все это продолжилось? Год 2212-й и далее, по 2345-й включительно. Убежища, одно за другим, выходят из подчинения главному. Восстание следует за восстанием, и каждое жестоко подавляется активацией ядовитого газа дистанционно. \nЧем все это закончится? Неизвестно. \nВ мае 2023 года состоялась премьера первого сезона телесериала «Укрытие», снятого по роману Хауи (режиссеры Адам Бернштейн и Мортен Тильдум по сценарию Грэма Йоста). Сериал пользовался огромной популярностью, получил высокие рейтинги и уже продлен на второй и третий сезоны.",
        discount: "31",
        exclusive: false,
        id: 3007976,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 979,
        originalPicture: "/nomenclature/30/079/3007976-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007976-2.jpg",
        price: 675,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "3.56",
          votes: 9,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Укрытие. Книга 2. Смена",
        url: "product/ukrytie-kniga-2-smena-3007976",
        yearPublishing: 2023,
      },
      id: "3007976",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Анастасия",
            id: 23159304,
            lastName: "Седова",
            middleName: "",
            url: "sedova-anastasiya",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110224,
          slug: "nauchno-populyarnaya-medicinskaya-literatura",
          title: "Научно-популярная медицинская литература",
          url: "nauchno-populyarnaya-medicinskaya-literatura",
        },
        coauthors: [],
        code: "3005400",
        description:
          "Вы когда-нибудь задумывались, с чем сталкиваются врачи-акушеры в своей практике? Как проходят их ночные дежурства? Как действуют врачи в сложных или безвыходных ситуациях?  \nАнастасия Седова – врач акушер-гинеколог, о случаях из своей практики рассказывает на Дзен-канале «Блог акушера». Канал стремительно набирает популярность, а один рассказ набирает более 50 тыс. дочитываний. \n«Дневник акушера. Реальные истории, рассказанные врачом роддома» – это уникальный сборник историй о работе акушеров, о сложных операциях и появлении новых людей на свет. В этой книге вы сможете узнать, кого и почему с такой надеждой ждет семнадцатилетняя цыганка, беременная в третий раз, успешно ли протекала беременность у 42-летней пациентки после многочисленных безуспешных попыток родить ребенка или что произошло с королевской двойней во время родов.  \nОт историй о радостных моментах появления на свет маленького человека до серьезных вызовов, когда надежды совсем не остается – эта книга вдохновит и заставит задуматься, как удивительна и хрупка жизнь в ранних ее проявлениях.",
        discount: "9",
        exclusive: false,
        id: 3005400,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 545,
        originalPicture: "/nomenclature/30/054/3005400-5.jpg",
        pages: "",
        picture: "/nomenclature/30/054/3005400-5.jpg",
        preOrderDate: "2023-11-01T00:00:00+03:00",
        price: 495,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.80",
          votes: 10,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Дневник акушера. Реальные истории, рассказанные врачом роддома",
        url: "product/dnevnik-akushera-realnye-istorii-rasskazannye-vrachom-roddoma-3005400",
        yearPublishing: 2023,
      },
      id: "3005400",
      type: "product",
    },
    {
      attributes: {
        authors: [],
        bestseller: false,
        binding: "",
        category: {
          id: 110025,
          slug: "muzykanty",
          title: "Музыканты",
          url: "muzykanty",
        },
        coauthors: [],
        code: "3009721",
        description:
          "Александр Красовицкий и Александр Заранкин проходятся по своим жизням от ранних воспоминаний до момента первого совместного выступления. Это самое глубокое публичное погружение в детство, в котором переплетаются максимально значимые и просто забавные эпизоды биографий, впервые публикуются фотографии из семейных альбомов. Рассказ всегда идёт от первого лица, но не всегда следует стройной хронологии — это больше дружеский разговор, чем летопись. Здесь же своими историями делятся и ближайшие родственники ребят, не только наблюдавшие за процессом их взросления, но и бывшие его непосредственными участниками.",
        discount: "17",
        exclusive: false,
        id: 3009721,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 799,
        originalPicture: "/nomenclature/30/097/3009721-1.jpg",
        pages: "",
        picture: "/nomenclature/30/097/3009721-1.jpg",
        preOrderDate: "2023-11-10T00:00:00+03:00",
        price: 660,
        publisher: {
          id: 120219,
          slug: "bombora",
          title: "БОМБОРА",
        },
        rating: {
          count: "4.60",
          votes: 10,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title:
          "Zero book. Двое из Animal ДжаZ — от первых детских воспоминаний до создания Zero People",
        url: "product/zero-book-dvoe-iz-animal-dzhaz-ot-pervyh-detskih-vospominaniy-do-sozdaniya-zero-people-3009721",
        yearPublishing: 2024,
      },
      id: "3009721",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Александр",
            id: 556976,
            lastName: "Бессонов",
            middleName: "",
            url: "bessonov-aleksandr",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110002,
          slug: "sovremennaya-proza",
          title: "Современная проза",
          url: "sovremennaya-proza",
        },
        coauthors: [],
        code: "3016389",
        description:
          'В четвертом сборнике Александра Бессонова — повесть в историях и письмах "Старшая по подъезду" и ранее не публиковавшиеся, но уже популярные, рассказы, прочитанные со сцены Павлом Деревянко, Максимом Матвеевым и другими резидентами литературно-театрального проекта "БеспринцЫпные чтения".',
        discount: "26",
        exclusive: false,
        id: 3016389,
        inSubscription: false,
        isBook: true,
        isBookmarks: true,
        new: true,
        oldPrice: 629,
        originalPicture: "/nomenclature/30/163/3016389-2.jpg",
        pages: "",
        picture: "/nomenclature/30/163/3016389-2.jpg",
        preOrderDate: "2023-11-24T00:00:00+03:00",
        price: 464,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.00",
          votes: 4,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Старшая по подъезду",
        url: "product/starshaya-po-podezdu-3016389",
        yearPublishing: 2024,
      },
      id: "3016389",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Ася",
            id: 449473,
            lastName: "Казанцева",
            middleName: "",
            url: "kazanceva-asya",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110295,
          slug: "biologiya-cheloveka-antropologiya",
          title: "Биология человека. Антропология",
          url: "biologiya-cheloveka-antropologiya",
        },
        coauthors: [],
        code: "3006759",
        description:
          "Новорожденные котята умеют ползать, косатки — плавать, слонята — ходить. Человеческий детеныш появляется на свет неприспособленным и в первый год жизни требует огромных ресурсов.\n\nВырастить здорового и счастливого человека — и при этом не лишиться карьеры, благополучия и душевного равновесия — колоссальная работа, несмотря на достижения медицины и рост уровня жизни. В результате многие решают не заводить детей вовсе: все сложно, а на планете нас и так почти 8 миллиардов.\n\nНаучная журналистка, популяризатор науки и молодая мать Ася Казанцева очень удивилась бы, если бы пять лет назад ей сказали, что она так рано захочет ребенка. Эта ее книга, четвертая по счету, особенная: в ней автор, как всегда, профессионально и увлекательно, рассказывает о недавних исследованиях в области эмбриологии, неонатологии и экстракорпорального оплодотворения; о реальных и мифических рисках для матери и плода; о возможностях и ограничениях современной медицины. Но это не все. Это еще и личная история самого важного решения в жизни и начала большой любви.",
        discount: "25",
        exclusive: false,
        id: 3006759,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 900,
        originalPicture: "/nomenclature/30/067/3006759-2.jpg",
        pages: "",
        picture: "/nomenclature/30/067/3006759-2.jpg",
        preOrderDate: "2023-10-27T00:00:00+03:00",
        price: 675,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "3.43",
          votes: 46,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Откуда берутся дети?",
        url: "product/otkuda-berutsya-deti-3006759",
        yearPublishing: 2023,
      },
      id: "3006759",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Адам",
            id: 392651,
            lastName: "Нэвилл",
            middleName: "",
            url: "nevill-adam",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110007,
          slug: "mistika-uzhasy",
          title: "Мистика. Ужасы",
          url: "mistika-uzhasy",
        },
        coauthors: [],
        code: "3006889",
        description:
          "Том вместе с семьей переезжает в сельский коттедж. Дом достался им по дешевке, не в последнюю очередь потому, что его предыдущий владелец совершил самоубийство. И никто не знает почему. Уже через несколько дней Том понимает, что, вдобавок к ремонту и плесени на стенах, ему придется столкнуться с еще одной проблемой: крайне неприятными и откровенно враждебными соседями, пожилой четой по фамилии Мут. Вскоре ссоры и мелкие дрязги перерастают в нечто куда более зловещее. Том начинает подозревать, что пара сварливых стариков за забором повелевает неземными силами, темной магией, которая древнее странного леса вокруг. И теперь Тому придется сражаться не только за собственный рассудок, но и за жизнь своей семьи.",
        discount: "26",
        exclusive: false,
        id: 3006889,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 749,
        originalPicture: "/nomenclature/30/068/3006889-2.jpg",
        pages: "",
        picture: "/nomenclature/30/068/3006889-2.jpg",
        preOrderDate: "2023-10-26T00:00:00+03:00",
        price: 556,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.64",
          votes: 14,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Колдуны",
        url: "product/kolduny-3006889",
        yearPublishing: 2023,
      },
      id: "3006889",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Дарья",
            id: 5940955,
            lastName: "Орлова",
            middleName: "",
            url: "orlova-darya",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110211,
          slug: "fitnes-aehrobika",
          title: "Фитнес. Аэробика",
          url: "fitnes-aehrobika",
        },
        coauthors: [],
        code: "3006237",
        description:
          "Я в выходные: буду два часа в день в спортзале, есть только брокколи и никаких сладостей.\nТак же я по будням: разлепила глаза, выпила растворимый кофе с бутербродом, поплелась на работу, вечером кое-как доползла до кровати.\n\nСил ни на что не хватает. Зачастую даже на нормальный сон. Где взять энергию на себя и свое здоровье?\nТвоя волшебная палочка — мини-упражнения из этой книги, которые запустят механизмы клеточного омоложения! С ней ты:\n• узнаешь, как работает твой организм;\n• поймешь, как получить подтянутое тело и сияющую кожу без седьмого пота и диет;\n• сможешь быстро и легко скорректировать питание;\n• получишь готовую систему легких видеотренировок.\n\nАвторская система пошаговых мини-упражнений помогла уже 620 тысячам девушек и женщин. Она составит серьезную конкуренцию косметологам, спортзалам и аптечным провизорам. Готова проверить?\n\nКнига в твердой обложке. Не является учебником по медицине. Все рекомендации должны быть согласованы с лечащим врачом.",
        discount: "9",
        exclusive: false,
        id: 3006237,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 677,
        originalPicture: "/nomenclature/30/062/3006237-6.jpg",
        pages: "",
        picture: "/nomenclature/30/062/3006237-6.jpg",
        preOrderDate: "2023-11-03T00:00:00+03:00",
        price: 615,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.44",
          votes: 18,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title:
          "Только не с понедельника! Система мини-тренировок для тех, кто хочет подтянутое тело, молодое лицо и крепкое здоровье",
        url: "product/tolko-ne-s-ponedelnika-sistema-mini-trenirovok-dlya-teh-kto-hochet-podtyanutoe-telo-molodoe-lico-i-krepkoe-zdorove-3006237",
        yearPublishing: 2023,
      },
      id: "3006237",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Коскэ",
            id: 8287917,
            lastName: "",
            middleName: "",
            url: "koske",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3006888",
        description:
          "Хаос разрывает Эргастул на части. Получив ужасное сообщение, Марко отправляется спасать похищенную Констанс. Ему предстоит вновь погрузиться в давно забытое прошлое и вступить в отчаянную схватку с врагом, которого когда-то считал своим братом.",
        discount: "26",
        exclusive: false,
        id: 3006888,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 829,
        originalPicture: "/nomenclature/30/068/3006888-3.jpg",
        pages: "",
        picture: "/nomenclature/30/068/3006888-3.jpg",
        preOrderDate: "2023-10-16T00:00:00+03:00",
        price: 616,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.56",
          votes: 16,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Гангста. Gangsta. Том 7",
        url: "product/gangsta-gangsta-tom-7-3006888",
        yearPublishing: 2023,
      },
      id: "3006888",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Елена",
            id: 23190498,
            lastName: "Легран",
            middleName: "",
            url: "legran-elena",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110343,
          slug: "istoriya-izobrazitelnogo-iskusstva",
          title: "История изобразительного искусства",
          url: "istoriya-izobrazitelnogo-iskusstva",
        },
        coauthors: [],
        code: "3010338",
        description:
          'Книга "Разгадай код художника: новый взгляд на известные шедевры" — это настоящий проводник в мир живописи, загадку которой вы точно так и не раскрыли до сегодняшнего дня. QR-коды, наполняющие страницы книги, скрывают за собой главные шедевры известных художников разных времен. Автор книги — историк и искусствовед Елена Легран — предоставляет уникальную возможность слой за слоем расшифровать мотивы творцов. Автор дает подробную историческую справку и посвящает нас в удивительные события прежде, чем мы вместе познаем таинство каждой работы. Благодаря этой книге вы сможете сами, посещая различные экспозиции, находить символы в каждом мазке и цвете, вычленять причинно-следственные связи между мотивами художника и временем, в котором он жил, а также стать настоящим знатоком главных шифров мировой живописи. Если вы готовы рискнуть и быть посвященным в самые сокровенные истории художников, которые они отразили в своих картинах, — эта книга точно для вас!',
        discount: "17",
        exclusive: false,
        id: 3010338,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 879,
        originalPicture: "/nomenclature/30/103/3010338-2.jpg",
        pages: "",
        picture: "/nomenclature/30/103/3010338-2.jpg",
        preOrderDate: "2023-11-01T00:00:00+03:00",
        price: 726,
        publisher: {
          id: 120219,
          slug: "bombora",
          title: "БОМБОРА",
        },
        rating: {
          count: "5.00",
          votes: 3,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Разгадай код художника: новый взгляд на известные шедевры",
        url: "product/razgaday-kod-hudozhnika-novyy-vzglyad-na-izvestnye-shedevry-3010338",
        yearPublishing: 2023,
      },
      id: "3010338",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Алан",
            id: 6488418,
            lastName: "Чароит",
            middleName: "",
            url: "charoit-alan",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3006768",
        description:
          "По легенде, клеть птицы-войны отпирается ненавистью, а запирается только любовью. Но между Дивью и Навью нет любви и мира уже много десятилетий. Их война кажется бесконечной, а реки злобы и споров не собираются иссякать.\nНо даже в самой жестокой бойне есть место лучу света. Хватит ли его, чтобы остановить кровопролитие?",
        discount: "26",
        exclusive: false,
        id: 3006768,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 599,
        originalPicture: "/nomenclature/30/067/3006768-8.jpg",
        pages: "",
        picture: "/nomenclature/30/067/3006768-8.jpg",
        price: 443,
        publisher: {
          id: 125802,
          slug: "zhanry",
          title: "Жанры",
        },
        rating: {
          count: "4.24",
          votes: 17,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Кощеевич и война",
        url: "product/koshcheevich-i-voyna-3006768",
        yearPublishing: 2023,
      },
      id: "3006768",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Коте",
            id: 23167257,
            lastName: "Оганезов",
            middleName: "",
            url: "oganezov-kote",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110238,
          slug: "konservirovanie",
          title: "Консервирование",
          url: "konservirovanie",
        },
        coauthors: [],
        code: "3007045",
        description:
          "Новый год в нашей стране — особенный праздник. Самый долгожданный, самый семейный, самый объединяющий. И в своей дебютной книге Коте Оганезов — шеф-повар, популярный блогер и убежденный семьянин — вновь напоминает нам об этом. Он делится рецептами, которые переходили в его семье из поколения в поколение, и рассказывает, что готовят на праздник в его собственной семье. Вас ждут и грузинские долма, чахохбили, тбилисская шакшука, и всеми любимые холодец, солянка, запеченная утка. А кроме этого — целый ряд блюд, которые которые так вкусно готовить на расслабленных каникулах, и десерты, которые станут достойным завершением праздничной трапезы и ярким акцентом послепраздничного чаепития!",
        discount: "17",
        exclusive: false,
        id: 3007045,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 999,
        originalPicture: "/nomenclature/30/070/3007045-12.jpg",
        pages: "",
        picture: "/nomenclature/30/070/3007045-12.jpg",
        price: 834,
        publisher: {
          id: 152770,
          slug: "hlebsol",
          title: "ХлебСоль",
        },
        rating: {
          count: "4.47",
          votes: 47,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title:
          "Уютные новогодние рецепты: любимые праздничные блюда для самых близких",
        url: "product/uyutnye-novogodnie-recepty-lyubimye-prazdnichnye-blyuda-dlya-samyh-blizkih-3007045",
        yearPublishing: 2024,
      },
      id: "3007045",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Никки",
            id: 21604810,
            lastName: "Сент Кроу",
            middleName: "",
            url: "sent-krou-nikki",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "2981971",
        description:
          "Я чувствовала себя мёртвой внутри, пока Питер Пэн не забрал меня к себе и своим Потерянным Мальчишкам.\n Только среди них, в Неверленде, я наконец ощутила себя живой.\n Но в здешних лесах бродит нечто тёмное и зловещее, а королева фейри и капитан Крюк готовы сразиться с Пэном за власть над волшебной страной.\n Сможет ли Король Неверленда вернуть свою тень и свой трон? И если он сделает это, буду ли я рядом с ним?\n \n Продолжение нашумевшей серии «Порочные Потерянные Мальчишки».\n Страстный, тёмный, взрослый ретеллинг культовой истории о Питере Пэне, полный жарких сцен и неожиданных сюжетных поворотов.\n Высочайшие рейтинги на Amazon: неизменный бестселлер вот уже больше года!\n Внутренние иллюстрации популярной художницы ALES (больше 50 000 подписчиков в ВК).\n Уникальная обложка, которая светится в темноте.\n Целая россыпь харизматичных мужчин, невероятные эмоции, поцелуи, ссоры и чувственность в каждом прикосновении.",
        discount: "9",
        exclusive: false,
        id: 2981971,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 728,
        originalPicture: "/nomenclature/29/819/2981971-2.jpg",
        pages: "",
        picture: "/nomenclature/29/819/2981971-2.jpg",
        preOrderDate: "2023-10-29T00:00:00+03:00",
        price: 662,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "3.84",
          votes: 250,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Тень Тьмы",
        url: "product/ten-tmy-2981971",
        yearPublishing: 2023,
      },
      id: "2981971",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Юлия",
            id: 23166291,
            lastName: "Фим",
            middleName: "",
            url: "fim-yuliya",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3006892",
        description:
          "«Возрождение темной» — фэнтези о неравенстве общества и мести в китайских декорациях, рассказывающее о становлении злодейки.\n\nИстория вдохновлена Дейнерис и большой любовью к китайским дорамам.\nИмперия Чжао — альтернативный Китай прошлого, место со сложившимися традициями и порядками. В ней правят представители Светлой фракции, беловолосые легендарные герои страны, а представители темной фракции, красноглазые демоны, преследуются по закону.\nЧживэй, попаданка, получает наихудший расклад из возможных в этом мире, и она не планирует спасать мир или даже себя.\nОна просто хочет отомстить.",
        discount: "26",
        exclusive: false,
        id: 3006892,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 599,
        originalPicture: "/nomenclature/30/068/3006892-3.jpg",
        pages: "",
        picture: "/nomenclature/30/068/3006892-3.jpg",
        preOrderDate: "2023-10-26T00:00:00+03:00",
        price: 443,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.22",
          votes: 27,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Возрождение Тёмной",
        url: "product/vozrozhdenie-temnoy-3006892",
        yearPublishing: 2023,
      },
      id: "3006892",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Владимир",
            id: 570899,
            lastName: "Сорокин",
            middleName: "Георгиевич",
            url: "sorokin-vladimir",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110002,
          slug: "sovremennaya-proza",
          title: "Современная проза",
          url: "sovremennaya-proza",
        },
        coauthors: [],
        code: "3012567",
        description:
          "Будущее наступило и прошло, ядерная война почти забыта, однако мир так и не наступил: насилие стало нормой и потребностью, терпимость к нему заразней, чем боливийский вирус.\nВ заключительной части трилогии о докторе Гарине Владимир Сорокин рисует новую стадию постапокалиптического распада. Главный герой изменился до неузнаваемости, но, как и положено врачу, остается примером достоинства и человечности. А его необычные наследники дают надежду на то, что у обезумевшего мира есть не только конец, но и более счастливое продолжение.",
        discount: "25",
        exclusive: false,
        id: 3012567,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 899,
        originalPicture: "/nomenclature/30/125/3012567-2.jpg",
        pages: "",
        picture: "/nomenclature/30/125/3012567-2.jpg",
        price: 670,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "3.40",
          votes: 5,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Наследие",
        url: "product/nasledie-3012567",
        yearPublishing: 2024,
      },
      id: "3012567",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Кристофер",
            id: 570287,
            lastName: "Сэнсом",
            middleName: "Джон",
            url: "sensom-kristofer",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110002,
          slug: "sovremennaya-proza",
          title: "Современная проза",
          url: "sovremennaya-proza",
        },
        coauthors: [],
        code: "3007948",
        description:
          "История не терпит сослагательного наклонения, но что было бы, если... Британия в 1940-м, после Дюнкерка, капитулировала перед гитлеровской Германией? К. Дж. Сэнсом отвечает на этот вопрос в своем захватывающем альтернативно-историческом эпике «Доминион», не переосмысливая, а заново создавая прошлое. 1952 год. На востоке продолжается война между Германией и Россией, обескровливая мировую экономику. Принудительные наборы на работу вынуждают молодых людей во Франции, Италии и Испании в буквальном смысле прятаться по норам. Англичане на собственном опыте постигают все ужасы фашистского авторитаризма, но их неповиновение растет. Сопротивление, возглавляемое Черчиллем, упорно пытается сдвинуть баланс сил на мировой арене, и точкой опоры может послужить некая тайна, которую хранит ученый, запертый в бирмингемской больнице для умалишенных. По заданию Сопротивления его похищают, чтобы переправить за границу, однако гестапо отправляет по следам беглеца безжалостного, не знающего поражений охотника за людьми... Впервые на русском!",
        discount: "31",
        exclusive: false,
        id: 3007948,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1099,
        originalPicture: "/nomenclature/30/079/3007948-2.jpg",
        pages: "",
        picture: "/nomenclature/30/079/3007948-2.jpg",
        price: 758,
        publisher: {
          id: 116515,
          slug: "azbuka",
          title: "Азбука",
        },
        rating: {
          count: "4.67",
          votes: 9,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        status: "canBuy",
        title: "Доминион",
        url: "product/dominion-3007948",
        yearPublishing: 2023,
      },
      id: "3007948",
      type: "product",
    },
    {
      attributes: {
        authors: [],
        bestseller: false,
        binding: "",
        category: {
          id: 110016,
          slug: "lyubovnye-romany",
          title: "Любовные романы",
          url: "lyubovnye-romany",
        },
        coauthors: [],
        code: "2996348",
        description:
          "Соленый морской воздух, вечера у костра, завораживающие пейзажи и шторм, который нужно пережить, чтобы влюбиться друг в друга.\n\nДолгожданный немецкий цикл Карины Шнелль «Стихии любви» состоит из трех книг: «Когда приходит шторм», «Когда наступает ночь», «Когда сталкиваются звезды». Это истории любви на фоне невероятной природы не только увлекательны, но и полезны, потому что многие читатели увидят в главных героях себя. \n\nПокет большого формата с запечатанными клапанами, дополнительным контентом к книге, закрашенным обрезом и сияющей фольгой. Идеальный подарок для любителей качественных современных романов, которые хочется поставить в свой книжный шкаф на самое видное место.\n\n\n«Шелест дождевых струй, рокот волн и гулкие раскаты грома. Я навсегда запомню эти волшебные минуты под дождем и близость его тела. Эта стихия – не шторм. Эта стихия – любовь».\n\nМашина Марли ломается на въезде в маленький городок на берегу моря. Ремонт продлится не меньше двух недель, поэтому Марли вынуждена поселиться в гостинице и искать работу, чтобы оплатить ремонт и проживание.\nОна устраивается администратором в ветеринарную клинику и быстро находит новых друзей. А еще Марли знакомится с неразговорчивым Джеком, с которым совсем не готова обсуждать истинную причину своего приезда. Джек тоже не готов делиться тайнами своего прошлого. \nНо в один день все меняется. Герои застревают в старой хижине и начинается шторм. \n\n«Острая на язык Марли любит брендовые кроссовки и ненавидит алкоголь. Скрытный красавчик Джек похож на лесоруба и обожает своего ретривера. В крохотном городе Сент-Эндрюс их сводит сама судьба: и хотя сначала общение героев состоит из пикировок, взаимный интерес постепенно берет верх, перерастая в нечто восхитительное...» – АНТОНИНА КРЕЙН, писательница",
        discount: "9",
        exclusive: false,
        id: 2996348,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 759,
        originalPicture: "/nomenclature/29/963/2996348-3.jpg",
        pages: "",
        picture: "/nomenclature/29/963/2996348-3.jpg",
        preOrderDate: "2023-11-03T00:00:00+03:00",
        price: 690,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.09",
          votes: 101,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Когда приходит шторм",
        url: "product/kogda-prihodit-shtorm-2996348",
        yearPublishing: 2023,
      },
      id: "2996348",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Ольга",
            id: 7495850,
            lastName: "Птицева",
            middleName: "",
            url: "pticeva-olga",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3003688",
        description:
          "Леся — городская, она не помнит своего прошлого, не знает законов, по которым живет лес. Чужой дом стал для нее пристанищем, но злая ворожба и давние тайны, что жили в нем, не принесли покоя. Теперь Лесе предстоит долгий путь через лес, населенный не только дикими зверьми, но и диким людом. И мертвая берегиня ведет ее без тропы, и волк в человечьем обличии рычит за спиной, и тот, кто рожден не тем сыном, обещает всегда оставаться рядом.\nНо лес могуч, будь тих в нем и не верь обещаниям. Может, выживешь, девка. А может, и нет.\nВторая книга дилогии Ольги Птицевой «Хроники болотного края».",
        discount: "29",
        exclusive: false,
        id: 3003688,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 679,
        originalPicture: "/nomenclature/30/036/3003688-3.jpg",
        pages: "",
        picture: "/nomenclature/30/036/3003688-3.jpg",
        preOrderDate: "2023-10-19T00:00:00+03:00",
        price: 480,
        publisher: {
          id: 112912,
          slug: "popcorn-books",
          title: "Popcorn Books",
        },
        rating: {
          count: "3.81",
          votes: 43,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Брат болотного края: роман",
        url: "product/brat-bolotnogo-kraya-roman-3003688",
        yearPublishing: 2023,
      },
      id: "3003688",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "",
            id: 23194212,
            lastName: "Брауни",
            middleName: "",
            url: "brauni",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "2989034",
        description:
          '"Что есть молодость? Стыдливость, бунтарство, самоуверенность, избыток самоанализа и..." Нань Хао и Шан Фэн — лучшие друзья, которые живут на полную катушку. Невероятно пафосная, местами абсурдная и очень забавная история о том, как здорово найти человека, с которым ты на одной волне. Пусть и очень дурацкой.',
        discount: "9",
        exclusive: false,
        id: 2989034,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1268,
        originalPicture: "/nomenclature/29/890/2989034-3.jpg",
        pages: "",
        picture: "/nomenclature/29/890/2989034-3.jpg",
        preOrderDate: "2023-10-24T00:00:00+03:00",
        price: 1153,
        publisher: {
          id: 135192,
          slug: "mann-ivanov-i-ferber",
          title: "Манн, Иванов и Фербер",
        },
        rating: {
          count: "4.43",
          votes: 146,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title: "Нань Хао и Шан Фэн. Том 1",
        url: "product/nan-hao-i-shan-fen-tom-1-2989034",
        yearPublishing: 2023,
      },
      id: "2989034",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Таня",
            id: 7504724,
            lastName: "Свон",
            middleName: "",
            url: "svon-tanya",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3008371",
        description:
          "По древнему обычаю Руслану против воли выдают замуж, однако во время обряда начинается вьюга, в которой жених девушки, Войко, исчезает. В насланном урагане несправедливо обвиняют Руслану, городскую охотницу, считая ее лесной девкой — ученицей Борового.\n Руслана не готова мириться с несправедливым обвинением в колдовстве, тем более девушка знает — у Войко есть возлюбленная! К ней-то он и сбежал!\n Чтобы защитить свое имя, Руслана отправляется на поиски беглеца, но все оказывается не так просто...\n \n Пять причин купить:\n 1. Всем любителям славянского фэнтези!\n 2. Яркий, оригинальный авторский мир.\n 3. Привлекательная и сильная главная героиня.\n 4. Увлекательный, захватывающий сюжет.\n 5. Неоднозначный, мифологичный, вряд ли счастливый, но справедливый финал.",
        discount: "9",
        exclusive: false,
        id: 3008371,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 623,
        originalPicture: "/nomenclature/30/083/3008371-5.jpg",
        pages: "",
        picture: "/nomenclature/30/083/3008371-5.jpg",
        preOrderDate: "2023-11-01T00:00:00+03:00",
        price: 566,
        publisher: {
          id: 154486,
          slug: "eksmo",
          title: "Эксмо",
        },
        rating: {
          count: "4.24",
          votes: 25,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Сердцецветы для охотницы",
        url: "product/serdcecvety-dlya-ohotnicy-3008371",
        yearPublishing: 2023,
      },
      id: "3008371",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Ники",
            id: 18635604,
            lastName: "Сью",
            middleName: "",
            url: "syu-niki",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110016,
          slug: "lyubovnye-romany",
          title: "Любовные романы",
          url: "lyubovnye-romany",
        },
        coauthors: [],
        code: "3008435",
        description:
          "Их история закончилась на выпускном. Любовь растворилась, оставив после себя лишь пепел. С тех пор прошло три года. Рита вырвалась из-под гнета родителей, обрела уверенность в себе, у нее появился лучший друг. Жизнь Вити тоже не стояла на месте. Он стал помогать отцу на работе, нашел очаровательную спутницу и, казалось, забыл о предательстве Риты.\n\nНо одним декабрьским днем все перевернулось: Витя вновь ее увидел. Свою подругу детства. Девушку, растоптавшую его чувства несколько лет назад. Кажется, у Риты есть большой секрет, о котором юноша даже не подозревал.\n\nСможет ли Витя узнать, что произошло в тот роковой выпускной вечер? Примет ли тяжелую правду, положившую конец романтическим отношениям? И есть ли у их истории шанс на счастливый финал?",
        discount: "18",
        exclusive: false,
        id: 3008435,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 729,
        originalPicture: "/nomenclature/30/084/3008435-2.jpg",
        pages: "",
        picture: "/nomenclature/30/084/3008435-2.jpg",
        preOrderDate: "2023-11-23T00:00:00+03:00",
        price: 600,
        publisher: {
          id: 109138,
          slug: "freedom",
          title: "Freedom",
        },
        rating: {
          count: "4.87",
          votes: 23,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Первые чувства. Искры снега",
        url: "product/pervye-chuvstva-iskry-snega-3008435",
        yearPublishing: 2024,
      },
      id: "3008435",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Елена",
            id: 569504,
            lastName: "Михалкова",
            middleName: "Ивановна",
            url: "mihalkova-elena",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110011,
          slug: "detektivnyj-roman-triller",
          title: "Детективный роман. Триллер",
          url: "detektivnyj-roman-triller",
        },
        coauthors: [],
        code: "3008274",
        description:
          'Полина Журавлева и Александра Нечаева живут в параллельных мирах. Одна — москвичка, любимица своих близких. Другая — провинциалка, ребенок из неблагополучной семьи, не сохранившая ни друзей, ни привязанностей.\nНо точка пересечения этих миров — жестокое убийство, погружающее в темные и страшные тайны.\nЕсли что-то на первый взгляд кажется тебе милым, ясным и привлекательным, остановись. Отвернись. Посмотри ещё раз. Вглядись пристально. Прислушайся к своей интуиции. Если она кричит: "Спасайся!" — не жди, пока на помощь придут частные детективы Макар Илюшин и Сергей Бабкин.\nОни могут не успеть.',
        discount: "9",
        exclusive: false,
        id: 3008274,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 677,
        originalPicture: "/nomenclature/30/082/3008274-5.jpg",
        pages: "",
        picture: "/nomenclature/30/082/3008274-5.jpg",
        preOrderDate: "2023-11-14T00:00:00+03:00",
        price: 615,
        publisher: {
          id: 125802,
          slug: "zhanry",
          title: "Жанры",
        },
        rating: {
          count: "5.00",
          votes: 5,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Посмотри, отвернись, посмотри",
        url: "product/posmotri-otvernis-posmotri-3008274",
        yearPublishing: 2023,
      },
      id: "3008274",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "TurtleMe",
            id: 20106330,
            lastName: "",
            middleName: "",
            url: "turtleme",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110064,
          slug: "manga",
          title: "Манга",
          url: "manga",
        },
        coauthors: [],
        code: "3003273",
        description:
          "Артур, бывший король процветающего государства, переродившийся в мире заклинателей и усилителей, попал в плен к бандитам. Ему открывается шокирующая правда, способная все изменить. Теперь у Артура только одно желание – защитить свою семью. Вскоре он понимает, что одного ядра маны будет недостаточно, чтобы уберечь всех от опасности, даже со знаниями и накопленным опытом прошлой жизни, ведь континент Дикатен – пугающее место. Когда на пути Артура возникнут мифические существа и работорговцы, ему придется приложить немало усилий, чтобы выжить и вернуться домой.",
        discount: "9",
        exclusive: false,
        id: 3003273,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1101,
        originalPicture: "/nomenclature/30/032/3003273-3.jpg",
        pages: "",
        picture: "/nomenclature/30/032/3003273-3.jpg",
        preOrderDate: "2023-11-14T00:00:00+03:00",
        price: 1001,
        publisher: {
          id: 109138,
          slug: "freedom",
          title: "Freedom",
        },
        rating: {
          count: "4.20",
          votes: 35,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Начало после конца. Том 2",
        url: "product/nachalo-posle-konca-tom-2-3003273",
        yearPublishing: 2024,
      },
      id: "3003273",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Александр",
            id: 589504,
            lastName: "Афанасьев",
            middleName: "Николаевич",
            url: "afanasev-aleksandr",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110082,
          slug: "literaturovedenie-folkloristika",
          title: "Литературоведение. Фольклористика",
          url: "literaturovedenie-folkloristika",
        },
        coauthors: [],
        code: "2999741",
        description:
          "В этой книге собраны русские народные сказки с яркими женскими персонажами. Каждая героиня является отражением определенного архетипа, разглядев который, можно по-новому осознать привычные истории. \nВоительница Марья Моревна, величественная Елена Премудрая, коварная Лисичка-сестричка, зачарованная Царевна-лягушка и другие героини оживают под рукой талантливой художницы Аnteikovich, которая подарила новый облик старым сказкам.",
        discount: "9",
        exclusive: false,
        id: 2999741,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 1154,
        originalPicture: "/nomenclature/29/997/2999741-5.jpg",
        pages: "",
        picture: "/nomenclature/29/997/2999741-5.jpg",
        preOrderDate: "2023-11-10T00:00:00+03:00",
        price: 1049,
        publisher: {
          id: 135192,
          slug: "mann-ivanov-i-ferber",
          title: "Манн, Иванов и Фербер",
        },
        rating: {
          count: "4.41",
          votes: 32,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title:
          "Русские народные сказки с женскими архетипами. Баба-яга, Марья Моревна, Василиса Премудрая и другие героини",
        url: "product/russkie-narodnye-skazki-s-zhenskimi-arhetipami-baba-yaga-marya-morevna-vasilisa-premudraya-i-drugie-geroini-2999741",
        yearPublishing: 2023,
      },
      id: "2999741",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Анна",
            id: 23187804,
            lastName: "Читанава",
            middleName: "",
            url: "chitanava-anna",
          },
        ],
        bestseller: true,
        binding: "",
        category: {
          id: 110224,
          slug: "nauchno-populyarnaya-medicinskaya-literatura",
          title: "Научно-популярная медицинская литература",
          url: "nauchno-populyarnaya-medicinskaya-literatura",
        },
        coauthors: [],
        code: "3006236",
        description:
          "Дополнительные конечности, второй мозг, глаза с лазерами, чипы отслеживания по всему телу и ежедневные ванны из плазмы крови. Кажется, именно так мы представляем биохакинг. Оставим фантазии фантастам. \nПропатчите и апгрейдите здоровье вашего ребенка! Биохакинг — это просто. С научной точки зрения сегодня это оптимизация имеющихся ресурсов организма для здоровой и насыщенной жизни. Врач-биохимик, специалист по витаминам, прошедшая обучение и стажировку в России, Германии, Китае и США, Анна Читанава расскажет о химической жизни организма и о том, как мы можем подружить детское тело с витаминами, жирами, углеводами и белками, а также помочь ему стать здоровее по всем показателям. Все рекомендации даны с опорой на профессиональный опыт автора и ведущих экспертов в области биохимии и нутрициологии (в книге указаны более 150 работ, на которые опирался автор). У вас уже есть супермен! Помогите ему стать здоровым, счастливым и долететь до звезд. \nNB! Не является учебником по медицине. Все рекомендации должны быть согласованы с лечащим врачом.",
        discount: "26",
        exclusive: false,
        id: 3006236,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 779,
        originalPicture: "/nomenclature/30/062/3006236-5.jpg",
        pages: "",
        picture: "/nomenclature/30/062/3006236-5.jpg",
        preOrderDate: "2023-10-27T00:00:00+03:00",
        price: 578,
        publisher: {
          id: 118732,
          slug: "ast",
          title: "АСТ",
        },
        rating: {
          count: "4.78",
          votes: 9,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Октябрь 2023",
        status: "canBuy",
        title:
          "Биохакинг. Дети. Пищевой блокбастер. Доказательная медицина и здоровье ребенка: от витаминов до болезней",
        url: "product/biohaking-deti-pishchevoy-blokbaster-dokazatelnaya-medicina-i-zdorove-rebenka-ot-vitaminov-do-bolezney-3006236",
        yearPublishing: 2024,
      },
      id: "3006236",
      type: "product",
    },
    {
      attributes: {
        authors: [
          {
            firstName: "Рэйчел",
            id: 568199,
            lastName: "Кейн",
            middleName: "",
            url: "keyn-reychel",
          },
        ],
        bestseller: false,
        binding: "",
        category: {
          id: 110005,
          slug: "fehntezi",
          title: "Фэнтези",
          url: "fehntezi",
        },
        coauthors: [],
        code: "3007643",
        description:
          "Армия Уэльса нарушила договор о перемирии и разграбила ценные книги. Теперь Библиотека будет жестоко наказывать всех, кто проявляет непокорность. \nДжесс Брайтвелл и его друзья бежали из Лондона, но попали в плен к поджигателям. Эта опасная группировка собирается уничтожить Библиотеку и остановить ее многолетнюю тиранию. Ради достижения своей цели они готовы бросать в огонь неповинных людей. \nНо, несмотря на все зло, Великая библиотека слишком ценна, чтобы позволить ей погибнуть. Поэтому Джесс с друзьями решают ее спасти и построить печатную машину, которая сделает книги доступными для всех. \nУспеют ли они осуществить свой план? Времени совсем не осталось.",
        discount: "18",
        exclusive: false,
        id: 3007643,
        inSubscription: false,
        isBook: true,
        isBookmarks: false,
        new: true,
        oldPrice: 699,
        originalPicture: "/nomenclature/30/076/3007643-3.jpg",
        pages: "",
        picture: "/nomenclature/30/076/3007643-3.jpg",
        preOrderDate: "2023-11-16T00:00:00+03:00",
        price: 576,
        publisher: {
          id: 111151,
          slug: "like-book",
          title: "Like Book",
        },
        rating: {
          count: "5.00",
          votes: 2,
        },
        recommended: false,
        sale: false,
        saleSoon: false,
        startSale: null,
        startSaleDesc: "Ноябрь 2023",
        status: "canBuy",
        title: "Пепел и перо",
        url: "product/pepel-i-pero-3007643",
        yearPublishing: 2024,
      },
      id: "3007643",
      type: "product",
    },
  ],
  meta: {
    pagination: {
      count: 48,
      current_page: 1,
      per_page: 48,
      total: 261,
      total_pages: 6,
    },
  },
};
export default novelty;
